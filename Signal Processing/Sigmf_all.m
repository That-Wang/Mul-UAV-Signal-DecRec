...
%% 生成数据集与标签
    ...
    for m = 1:da_snr_num
        ...
        for k = 1:numPerSig  % 变换信号信噪比，保存信号定位信息
            if m ==1  % 遍历所有snr
                snrlist(k) = randsample(snrpool,1);
            else
                ...
                if index_snr < length(snrpool)
                    ...
                    snrlist(k) = snrpool(1);  % Wrap around to the first value
                end
            end
...
% % % % %             else
% % % % %                 fid_dec_snr = fopen(sprintf('D:\\download\\UAVDataset\\dectest\\labels_dec_snr\\UAVSig%d_DA%d.txt', i, m), writeMark);
% % % % %                 fid_decrec_snr = fopen(sprintf('D:\\download\\UAVDataset\\dectest\\labels_decrec_snr\\UAVSig%d_DA%d.txt', i, m), writeMark);
% % % % %             end
...
% % % % %             end
        ...
        elseif i > round(numSample/10*6) && i <= round(numSample/10*8)  
            ...
            imwrite(im, sprintf('D:\\download\\UAVDataset\\dectest\\images\\UAVSig%d_DA%d.png', i, m));
        end
    %     imshow(im);  % 显示图像
    %     title('灰度图像'); 
...
%                 end
    ...
    %    FinalSig = single(FinalSig);
        if i > round(numSample/10*8)
            save(sprintf('D:\\download\\UAVDataset\\alltest\\UAVSig%d_DA%d.mat', i, m), 'FinalSig');
        end
    end
end
